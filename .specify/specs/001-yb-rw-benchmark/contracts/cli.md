...
- `--generate <int>`: Optional. Number of rows to generate into the CSV file before execution. Will overwrite the file.
...
- `--mode <string>`: Required. Defines the benchmark mode:
  ...
  - `read_optimized`: Optimized range query (Sorted Partial Covering Indexes).
...
**Write Benchmarks**
```bash
# Generate 1M rows and test standard copy
python src/main.py --mode single --file data.csv --generate 1000000

# Test parallel copy with 20 workers
python src/main.py --mode parallel --file data.csv --workers 20
```

**Read Benchmarks**
```bash
# Test read latency and throughput on standard table for 60 seconds
python src/main.py --mode read_standard --file data.csv --workers 10 --duration 60 --no-init

# Test read latency and throughput on optimized indexes
python src/main.py --mode read_optimized --file data.csv --workers 10 --duration 60 --no-init
```
